...
EXPORT void testHashParamUnit() {
    ...
    if (item) {
        INFO_LOG("[%d] Param is %p\n", key, item->paramHash);
    } else {
        ERR_LOG("Cannot find parameter!");
    }
...
EXPORT APP_STATUS testAudioTypeLockFun(AppHandle *appHandle) {
    ...
    /* Save XML */
    audioTypeSaveAudioParamXml(audioType, CUST_XML_DIR, 1);
...
EXPORT APP_STATUS testAppHandleInitUninit() {
    ...
    for (i = 0; i < times; i++) {
        ...
        if (xmlDirFromProperty) {
            appHandleParseXml(&testAppHandle, (const char**)xmlDirFromProperty, CUST_XML_DIR);
        } else {
            appHandleParseXml(&testAppHandle, XML_FOLDER_LIST_ON_DEVICE, CUST_XML_DIR);
        }
#endif
